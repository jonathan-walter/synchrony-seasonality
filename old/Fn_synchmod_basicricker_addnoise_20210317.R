synchmod_basicricker_addnoise<-function(r, K, rho.b, rho.w, sd.b, sd.w, Db=0, 
                                   tmax=100, nlocs=2, ret.eb=FALSE, ret.ew=FALSE)
{
  ...
  #run model
  for(tt in 2:tmax){
    #Nt[,tt] = Nt[,(tt-1)]*exp(r*(1-(Nt[,(tt-1)]/K))) + eb[,tt] - abs(ew[,tt]) #should alternative model mimic action of ew?
    Nt[,tt] = Nt[,(tt-1)]*exp(r*(1-(Nt[,(tt-1)]/K))) + eb[,tt] + abs(ew[,tt])
