...
hypercube<-as.matrix(read.csv("~/GitHub/synchrony-seasonality/params_hypercube_n250_k7.csv"))
...
# sens.same<-mcmapply(synchsim_same, r=r, Kb=Kb, Kw=Kw, rho.b=rho.b, rho.w=rho.w,
#                        sd.b=sd.b, sd.w=sd.w, Db=Db, Dw=Dw, tmax=tmax, nlocs=nlocs, SIMPLIFY = F)
# sens.noow<-mcmapply(synchsim_noow, r=r, Kb=Kb, Kw=Kw, rho.b=rho.b, rho.w=rho.w,
#                        sd.b=sd.b, sd.w=sd.w, Db=Db, Dw=Dw, tmax=tmax, nlocs=nlocs, SIMPLIFY = F)
stopCluster(cl)
...
lm.samp.main<-lm(rhoN~r+Kw+Kb+rho.b+rho.w+sd.b+sd.w+sd.b*rho.b+sd.w*rho.w+rho.b*rho.w+Kw*Kb, data=sampres.main, na.action=na.exclude)
summary(lm.samp.main)

# lm.samp.same<-lm(rhoN~r+Kw+Kb+rho.b+rho.w+sd.b+sd.w+sd.b*rho.b+sd.w*rho.w+rho.b*rho.w+Kw*Kb, data=sampres.main, na.action=na.exclude)
# summary(lm.samp.same)
# 
# lm.samp.noow<-lm(rhoN~r+Kw+Kb+rho.b+rho.w+sd.b+sd.w+sd.b*rho.b+sd.w*rho.w+rho.b*rho.w+Kw*Kb, data=sampres.main, na.action=na.exclude)
...
par(mar=c(7,5,2,2))
barplot(lm.samp.rhoN$coefficients[-1], las=2, ylab = "Synchrony")
text(seq(from=0.6,by=1.2,length.out=11),lm.samp.rhoN$coefficients[-1],paste0("p=",round(modsumm.add$coefficients[-1,4],2)),pos=3,offset=0.5)
dev.off()
...
png(paste0("~/GitHub/synchrony-seasonality/fig3_compare_seasons.png"), units="in", res=300, width=6.5, height=4.5)

par(mfrow=c(2,3), mar=c(1.8,1.8,0.5,0), mgp=c(2.7,0.5,0), tcl=-0.3, oma=c(1.1,2.7,1.1,1))
#main model
...
image(rho, rho, CV.noow, xlab="", ylab="", asp=1, main="",
      col=viridis(50))
...
png(paste0("~/GitHub/synchrony-seasonality/fig3_compare_seasons_ereK.png"), units="in", res=300, width=6.5, height=4.5)
...
png("~/GitHub/synchrony-seasonality/fig4_compare_alternates_ereK.png", units="in", res=300, width=6, height=4.42)
...
png(paste0("~/GitHub/synchrony-seasonality/fig3_compare_seasons_eN.png"), units="in", res=300, width=6.5, height=4.5)
...
png("~/GitHub/synchrony-seasonality/fig4_compare_alternates_eN.png", units="in", res=300, width=6, height=4.42)
