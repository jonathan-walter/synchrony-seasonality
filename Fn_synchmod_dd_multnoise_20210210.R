synchmod_dd_multnoise<-function(r, Kb, Kw, rho.b, rho.w, sd.b, sd.w, Db=0, Dw=0, tmax=100, nlocs=2,
                               ret.Bt=TRUE, ret.eb=FALSE, ret.ew=FALSE)
{
  ...
  for(tt in 2:tmax){
    ...
    Wt[,tt] = max(0, Bt[,tt]/Kw-1  - abs(ew[,tt]))*Bt[,tt]#Winter mortality
    #Wt[,tt] = (Bt[,tt]/Kw-1  + ew[,tt])*Bt[,tt]#Winter mortality
    Nt[,tt] = Bt[,tt]-Wt[,tt]
