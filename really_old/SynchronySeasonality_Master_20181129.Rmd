...
```{r linear.sensitivity, echo=FALSE, cache=TRUE}
library(parallel)
...
linear.sample<-mcmapply(popSynchByStage.linear,beta=beta,sd.b=sd.b,rho.b=rho.b,Sw=Sw,sd.w=sd.w,rho.w=rho.w,
                            tmax=tmax,nlocs=nlocs, SIMPLIFY = F)
stopCluster(cl)

getNtcor<-function(inlist){
...
sampdf.lin<-data.frame(synch=getNtcor(linear.sample),beta=scale(beta),sd.b=scale(sd.b),rho.b=scale(rho.b),
                   Sw=scale(Sw),sd.w=scale(sd.w),rho.w=scale(rho.w))#scale to standardize effect sizes

effectmod.lin<-lm(synch~beta+sd.b+rho.b+Sw+sd.w+rho.w+sd.b*rho.b+sd.w*rho.w+rho.b*rho.w, data=sampdf.lin)
modsumm.lin<-summary(effectmod.lin)
### work out some plotting code for effect sizes
# tiff("~/Box Sync/SynchronyLifestageModel/plotoutput/effectsizes_linear.tif", units="in", res=300, width=6.5, height=3.25)
# par(mar=c(7,5,2,2))
# barplot(effectmod.lin$coefficients[-1], las=2, main="Effect sizes, linear model", ylim=c(0,0.25))
# text(seq(from=0.6,by=1.2,length.out=9),effectmod.lin$coefficients[-1],paste0("p=",round(modsumm$coefficients[-1,4],2)),pos=3,offset=0.5)
# dev.off()

save(modsumm.lin, file="modsumm_liner.RData")

```

# Effects of seasonality on synchrony and extinction, Ricker dynamics 

```{r ricker.sensitivity, echo=FALSE, cache=TRUE}
rm(list=ls())
library(parallel)
...
rho.b=runif(nn,0,1)
rho.w=runif(nn,0,1)
sd.b=Kb/runif(nn,3,10)
sd.w=Kb/runif(nn,3,10)
Db=runif(nn,0,0.4)
Dw=0
tmax=550
nlocs=10
...
# tiff("~/Box Sync/SynchronyLifestageModel/plotoutput/effectsizes_densdep_add.tif", units="in", res=300, width=6.5, height=3.25)
# par(mar=c(7,5,2,2))
...
save(modsumm.add, file="modsumm_additive.RData")
...
rho.b=runif(nn,0,1)
rho.w=runif(nn,0,1)
sd.b=runif(nn,0,0.3)#Kb/runif(nn,3,10)
sd.w=runif(nn,0,0.3)#Kb/runif(nn,3,10)
Db=runif(nn,0,0.4)
Dw=0
tmax=550
nlocs=10
...
cl<-makeCluster(ncores)
varrhos.lin<-mcmapply(popSynchByStage.linear,beta=beta,sd.b=sd.b,rho.b=rho.b,Sw=Sw,sd.w=sd.w,rho.w=rho.w,
                            tmax=tmax,nlocs=nlocs, SIMPLIFY = F)
stopCluster(cl)
...
Ntcor.df<-data.frame(rho.b=rho.b, rho.w=rho.w, Ntcor=getNtcor(varrhos.lin))
Ntcor.agg<-aggregate(Ntcor.df, by=list(rho.b, rho.w), FUN=mean)
...
diffs<-avg.mat-Ntcor.mat

pal<-colorRampPalette(colors=c("red","white","blue"))


png("~/Box Sync/SynchronyLifestageModel/plotoutput/fig2_stagesynchrony_linear.png", units="in", res=300, width=5.85, height=3.25)

par(mfrow=c(1,2), mar=c(1.8,1.8,1.5,0), mgp=c(2.7,0.5,0), tcl=-0.3, oma=c(1.1,1,0,1),
    cex.main=0.8,cex.axis=0.8)

image(rho, rho, Ntcor.mat, xlab="", ylab="", asp=1, main="Population synchrony",
      col=pal(50), zlim=c(-1,1))
contour(rho, rho, Ntcor.mat, add=T)
text(0.05,0.95,"a)")

image(rho, rho, diffs, xlab="", ylab="", asp=1, main="Difference from alternate",
      col=pal(50), zlim=c(-.15,.15))
contour(rho,rho,diffs,add=T)
text(0.05,0.95,"b)")

mtext("Spatial synchrony of breeding season environment",1,cex=1,outer=T)
mtext("Overwintering synchrony",2,cex=1,outer=T)
...
png(paste0("~/Box Sync/SynchronyLifestageModel/plotoutput/fig3_compare_seasons.png"), units="in", res=300, width=6.1, height=2.3)
...
# tiff(paste0("~/Box Sync/SynchronyLifestageModel/plotoutput/compare_vs_alt_",scentxt,".tif"), units="in", res=300, width=6.5, height=6.5)
# 
...
# sm<-matrix(predict(loess(z~x+y, data=tmpdf, span=0.1)),length(rho),length(rho))
# contour(rho, rho, sm, add=T, levels=c(0.1,0.12,0.14,0.16,0.18,0.2))
# 
...
# 
# 
# tiff(paste0("~/Box Sync/SynchronyLifestageModel/plotoutput/compare_vs_basic_",scentxt,".tif"), units="in", res=300, width=6.5, height=6.5)
# 
# par(mfcol=c(3,3), mar=c(4,4,1.5,1), mgp=c(2.7,1,0), tcl=-0.4)
# #main model
# image(rho, rho, Ntcor.mat, xlab="Breeding synchrony", ylab="Overwintering synchrony", asp=1, main="main cor(Nt)",
...
png(paste0("~/Box Sync/SynchronyLifestageModel/plotoutput/fig4_comparison_addnoise_",scentxt,".png"), units="in", res=300, width=6, height=6.55)
